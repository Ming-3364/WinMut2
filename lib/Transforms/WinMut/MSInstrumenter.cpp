  ...
  if (F.getName().equals("main")) {
    ...
    if (!finit) {
      ...
      FunctionType *Fty = FunctionType::get(
          Type::getVoidTy(TheModule->getContext()), Fty_args, true);
      finit = Function::Create(Fty,                          // Type
    ...
    CallInst *call_init =
        CallInst::Create(finit, "", &*F.getEntryBlock().begin());
...
void MSInstrumenter::filtMutsByIndex(Function &F, vector<Mutation *> *v) {
  ...
  while (cur_mut != v->end()) {
    if ((*cur_mut)->index != (*beg)->index) {
      //			errs()<<"IR "<<(*beg)->index<<" : mut
      //"<<(*beg)->id<<" ~~ mut "<<(*(cur_mut-1))->id<<"\n";
      //errs()<<" II :
      //"<<instrumented_insts<<"\n";

      int insts = instrument(F, v, (*beg)->index, (*beg)->id,
                             (*(cur_mut - 1))->id, instrumented_insts);
    ...
    if (cur_mut == (v->end() - 1)) {
      ...
      int insts = instrument(F, v, (*beg)->index, (*beg)->id, (*cur_mut)->id,
                             instrumented_insts);
      instrumented_insts += insts;
      //			errs()<<" III : "<<instrumented_insts<<"\n";
    }
...
int MSInstrumenter::instrument(Function &F, vector<Mutation *> *v, int index,
                               int mut_from, int mut_to,
                               int instrumented_insts) {
  int insts = 0;

  BasicBlock::iterator cur_it =
      MutUtil::getLocation(F, instrumented_insts, index);

  errs() << ">>INSTRUMENTING MUTS FROM " << mut_from << " TO " << mut_to
         << " IN IR " << index << " >> ";
  // cur_it->dump();
...
static RegisterPass<MSInstrumenter>
    X("AccMut-schemata", "AccMut - Instrument Code to Mutation Schemata");
/*-----------------reserved end --------------------*/
